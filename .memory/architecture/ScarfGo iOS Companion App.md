...
- [scope] Feature surface: multi-server, project-scoped chat, session resume, memory editor, cron list, skills tree, Kanban board, Curator, settings (read-only). All sessions are scoped to a project via the same Scarf-managed AGENTS.md block the Mac app writes. #features
