...
Phase 5 of projects-first-class (branch feat/projects-first-class, t-3d915f7f). A stdio MCP server shipped inside Scarf.app gives LOCAL Hermes agents six project tools — `project_list`, `project_get`, `project_register`, `project_update_dashboard`, `project_add_slash_command`, `project_validate` — replacing the `scarf-template-author` skill's "read projects.json, append your row, write it back" step that produced the 2026-09-02 corruption.
