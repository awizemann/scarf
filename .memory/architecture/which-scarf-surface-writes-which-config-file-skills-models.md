...
- [gotcha] Scarf's Skills → Uninstall passes `skill.id` (`<category>/<name>`) to a CLI that accepts only the BARE name, and the CLI's rejection EXITS 0 — so `finishUninstall` reported success and nothing was removed. Verified against v0.21; FIXED 2026-09-08 (t-ec6d2e6d): the view passes the bare name and `uninstallSucceeded(exitCode:output:)` treats an `Error:` line as failure. `--yes` exists on v0.21 but not v0.20, so the stdin "y" confirmation stays #skills #gotcha
