...
- [gotcha] `yamlScalar` escapes `\\` and `\"` when it quotes; `unquote` used to strip the quotes and stop, so `/a\b` became `/a\\b` one save later and again the save after — a permanent "the command moved". They are now inverses (double-quoted undoes `\\`/`\"` and leaves unknown escapes alone; single-quoted handles `''`).
