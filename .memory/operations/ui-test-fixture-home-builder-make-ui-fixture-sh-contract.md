...
- [fact] Seeds 3 real one-shot sessions, 2 paused cron jobs, 3 kanban cards (one blocked), 1 project, 1 official skill (`openhue`, offline), and `memories/MEMORY.md` + `USER.md`. Wall time 25s (plain) to ~75s (with `--self-check`), and it spends a few cents of real tokens because the sessions are genuine model calls #testing
...
- [decision] Never check a `state.db` into the repo — generating through the CLI keeps the schema matched to the Hermes the app actually faces and keeps charter C3 intact (Scarf never writes state.db; the fixture script does, through hermes) #decision
