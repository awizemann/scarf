...
@Suite struct CronRecoveryOfferP30Tests {
    ...
    private static func job(state: String, kind: String, enabled: Bool = false) throws -> HermesCronJob {
        let schedule: String
        switch kind {
        case "cron":     schedule = #"{"kind":"cron","expr":"0 9 * * *"}"#
        case "interval": schedule = #"{"kind":"interval","minutes":30}"#
        default:         schedule = #"{"kind":"once","run_at":"2099-01-01T09:00:00+00:00"}"#
        }
        let json = """
                ...
                for kind in ["once", "cron", "interval"] {
                    for state in ["scheduled", "paused", "completed", "error"] {
                        for enabled in [false, true] {
                            let j = try Self.job(state: state, kind: kind, enabled: enabled)
                            let a = mac.recoveryOffer(for: j)
                            let b = ios.recoveryOffer(for: j)
                            #expect(a == b,
                                    "\(state)/\(kind)/enabled=\(enabled) @ 0206=\(v0206) 021=\(v021): mac \(a) vs iOS \(b)")
                        }
                    }
                }
            }
